...
def send_not_approved_message(chat_id):
    bot.send_message(chat_id, "*YOU ARE NOT APPROVED OWNER @Mahakal1814*", parse_mode='Markdown')
...
def start_attack(message):
    ...
    if attack_details:
        ...
        bot.send_message(message.chat.id, f"*Daksh Ddos Attack started on Host: {target_ip} Port: {target_port}*", parse_mode='Markdown')
    else:
...
def stop_attack(message):
    ...
    if attack_details:
        ...
        bot.send_message(message.chat.id, f"*Daksh Ddos Attack stopped on Host: {target_ip} Port: {target_port}*", parse_mode='Markdown')
